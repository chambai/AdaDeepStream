...
def getData(isApplyDrift=True):
    ...
    if numUnseenInstances != -1:
        if isRandom:
            ...
            f_y_test_no_norm = f_y_test_no_norm[rndIdexes, :]
            if util.hasSubClasses():
                u_f_y_test = u_f_y_test[rndIdexes, :]
                u_f_y_test_no_norm = u_f_y_test_no_norm[rndIdexes, :]
        else:
            ...
            f_y_test_no_norm = f_y_test_no_norm[:numUnseenInstances]
            if util.hasSubClasses():
                u_f_y_test = u_f_y_test[:numUnseenInstances]
                u_f_y_test_no_norm = u_f_y_test_no_norm[:numUnseenInstances]
    ...
    if (dataDiscrepancy == 'none') or (dataDiscrepancy == 'AA'):
        ...
        if numUnseenInstances != -1:
            if isRandom:
                ...
                oof_y_test_no_norm = oof_y_test_no_norm[rndIdexes, :]
                if util.hasSubClasses():
                    u_oof_y_test = u_oof_y_test[rndIdexes, :]
                    u_oof_y_test_no_norm = u_oof_y_test_no_norm[rndIdexes, :]
            else:
                ...
                oof_y_test_no_norm = oof_y_test_no_norm[:numUnseenInstances]
                if util.hasSubClasses():
                    u_oof_y_test = u_oof_y_test[:numUnseenInstances]
                    u_oof_y_test_no_norm = u_oof_y_test_no_norm[:numUnseenInstances]
    ...
    for index in range(numDiscrepancy):
        ...
        if int(result) in dataDiscrepancyClass:
...
def EqualizeNumInstances(ndUnseenDataList, dUnseenDataList):
    ...
    # check that all expected classes are in the datastream
    expected_nd_classes = util.getParameter('DataClasses')
    discrep_classes_before = np.unique([d.correctResult for d in ndUnseenDataList])
    check_data(expected_nd_classes, discrep_classes_before, 'before')

    expected_discrep_classes = util.getParameter('DataDiscrepancyClass')
    discrep_classes_drift = np.unique([d.correctResult for d in dUnseenDataList])
    check_data(expected_discrep_classes, discrep_classes_drift, 'drift')
    ...
    if len(dUnseenRemainingDataList) == 0:
        if driftPattern == 'reoccurring':
            ...
            ndUnseenRemainingDataList = dUnseenDataList[(len(ndUnseenDataList) // 3) * 2:]
...
def applyDrift(ndUnseenDataList, dUnseenDataList):
    ...
    if driftPattern == 'none':
        ...
        if driftType == 'categorical':
            ...
            # mixed = np.asarray(itertools.chain(splitUnusedDataList))
            dUnseenDataList.extend(mixed)
            # for s in range(len(splitUnusedDataList)):
            #     for a in range(len(splitUnusedDataList[s])):
            #         d = splitUnusedDataList[s][a]
            #         dUnseenDataList.append(d)
        ...
        if do_equalize:
            dUnseenRemainingDataList = []
            # ndUnseenDataList, dUnseenDataList, dUnseenRemainingDataList = EqualizeNumInstances(ndUnseenDataList, dUnseenDataList)
        ...
        while stop == False and iCount < len(ndUnseenDataList) + len(dUnseenDataList):
            ...
            for d in range(numInst):
                # if iCount < (numDiscrepancy + numNonDiscrepancy):
                if dIndex < len(dUnseenDataList):
                    # add discrepancy
                    discrepancyInstance = dUnseenDataList[dIndex]
        ...
        # assign the last third of the stream as 'after' the drift
    ...
    u_dur = np.unique(during)
    after = [int(u.correctResult) for u in newUnseenDataList if u.adaptState == 'after']
    u_aft = np.unique(after)
    dataClasses = util.getParameter('DataClasses')
    discrep = util.getParameter('DataDiscrepancyClass')
    check_data(dataClasses, before, 'before')
    check_data(discrep, during, 'during')

    if driftPattern == 'reoccurring':
        # check original and new classes are in the data
        check_data(dataClasses, after, 'after')
        check_data(discrep, after, 'after')
    else:
        # if it's not reoccurring, just check that new classes are in the data after drift has occurred
        check_data(discrep, after, 'after')
