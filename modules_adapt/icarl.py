...
def setupAnalysis(indnn, act_train_batch, y_train_batch, inData=[]):
    ...
    indnn.model = analysis.set_dnn_output_features(indnn.model,
                    len(util.getParameter('DataClasses')) + len(util.getParameter('DataDiscrepancyClass')))  # have to give the total number of classes is in the dataset
    lr, wd = analysis.get_optimizer_params()

    ocl_mgr = OclMgr(indnn.model, 'ICARL', util.getParameter('DatasetName'), util.getParameter('Dnn'), util.getParameter('DataClasses'), accuracies.max(), lr, wd)
